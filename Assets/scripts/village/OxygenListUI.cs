...
public class OxygenListUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    [System.Serializable]
    ...
    [SerializeField] private List<OxygenEntry> entries = new List<OxygenEntry>();
    ...
    private void Awake()
    {
        ...
        if (purchaseButton != null)
            purchaseButton.onClick.AddListener(() => HandleInstall(false));
        if (installButton != null)
            installButton.onClick.AddListener(() => HandleInstall(true));
        if (closeButton != null)
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < spawnedItems.Count; i++)
        {
            ...
            bool owned = VillageManagement.Instance != null && VillageManagement.Instance.HasOwnedOxygen(entries[i].id);
            image.color = owned ? color : new Color(0.45f, 0.45f, 0.45f, color.a);
        }
    }

    public void Open(OxygenImplementation implementation)
    {
    ...
    public void OnDrag(PointerEventData eventData) => dragDelta = Mathf.Clamp((eventData.position.x - dragStartX) / spacing, -1f, 1f);
    ...
    private void HandleInstall(bool ownedAlready)
    {
        if (boundImplementation == null || entries.Count == 0)
            return;

        OxygenEntry entry = entries[selectedIndex];
        if (entry == null || entry.oxygenPrefab == null)
            return;

        boundImplementation.TryInstall(entry.oxygenPrefab, ownedAlready);
        Refresh();
    }
    ...
    private void Refresh()
    {
        ...
        OxygenEntry selected = entries[selectedIndex];
        bool owned = VillageManagement.Instance != null && VillageManagement.Instance.HasOwnedOxygen(selected.id);
        if (purchaseButton != null && VillageManagement.Instance != null)
            purchaseButton.interactable = !owned && VillageManagement.Instance.CurrentOxygen >= selected.oxygenPrefab.CurrentOxygenPrice;
        if (purchaseButton != null)
